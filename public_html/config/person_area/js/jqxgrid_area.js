...
function LoadAreasGrid(url) {
	...
	var source = {
		datatype: "json",
		datafields: [
			{ name: 'PersonArea_GUID',		display: 'PersonArea_GUID',		type: 'string' },
			{ name: 'PersonGUID',			display: 'PersonGUID',			type: 'string' },
			{ name: 'PersonDisplayName',	display: 'PersonDisplayName',	type: 'string' },
	...
	$("#jqxgrid2").jqxGrid({
		...
		renderstatusbar: function (statusbar) {
			...
			addButton.click(function (event) {
				if(!editModeG2 && !lockedForService) {
					...
					var newRow = {};
					newRow.PersonArea_GUID   = -1;
					newRow.PersonDisplayName = personName;
					newRow.PersonGUID = personGuid;
					
					grid2ChangesMade = true;
					
					$("#jqxgrid2").jqxGrid('addrow', null, newRow);
					$('#jqxgrid2').jqxGrid('selectrow', $('#jqxgrid2').jqxGrid('getrows').length - 1);
					$("#jqxgrid2").jqxGrid('begincelledit', $('#jqxgrid2').jqxGrid('getrows').length - 1, "AreaDisplayName");
				}
			});
		...
		columns: [
			...
			{ text: languagePack.personArea.grid2.PersonDisplayName, datafield: 'PersonDisplayName', editable: false },
			{ text: 'Area_GUID', datafield: 'Area_GUID', hidden: true, editable: false },
			{ text: languagePack.personArea.grid2.AreaDisplayName, datafield: 'AreaDisplayName', editable: true,  columntype: 'dropdownlist',
	...
	$("#jqxgrid2").on('cellendedit', function (event) {
		...
		if(value != oldvalue) {
			...
			if((dataField == 'AreaDisplayName') && rowData.PersonArea_GUID != -1) {
