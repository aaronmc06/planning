...
function LoadMeasuresGrid(url) {
	...
	var source = {
		datatype: "json",
		datafields: [
			{ name: 'Measure_GUID',					display: 'Measure_GUID',				type: 'string' },
			{ name: 'MeasureType_GUID',				display: 'MeasureType_GUID',			type: 'string' },
			{ name: 'MeasureCategory_GUID',			display: 'MeasureCategory_GUID',		type: 'string' },
			{ name: 'MeasureDisplayName',			display: 'MeasureDisplayName',			type: 'string' },
			{ name: 'MeasureTypeDisplayName',		display: 'MeasureTypeDisplayName',		type: 'string' },
			{ name: 'MeasureCategoryDisplayName',	display: 'MeasureCategoryDisplayName',	type: 'string' },
			{ name: 'IsLineupMeasure',				display: 'IsLineupMeasure',				type: 'bool'},
			{ name: 'Measure_IsActive',				display: 'Measure_IsActive',			type: 'bool' }
		],
		url: url
	};
	...
	$("#jqxgrid").jqxGrid({
		...
		renderstatusbar: function (statusbar) {
			...
			addButton.click(function (event) {
				if(!editModeG1 && !lockedForService) {
					...
					var newRow = {};
					newRow.Measure_GUID   = -1;
					newRow.IsLineupMeasure   = 0;
					
					grid1ChangesMade = true;
					
					$("#jqxgrid").jqxGrid('addrow', null, newRow);
					$('#jqxgrid').jqxGrid('selectrow', $('#jqxgrid').jqxGrid('getrows').length - 1);
					$("#jqxgrid").jqxGrid('begincelledit', $('#jqxgrid').jqxGrid('getrows').length - 1, "MeasureDisplayName");
				}
			});
		...
		columns: [
			...
			{ text: 'MeasureCategory_GUID', datafield: 'MeasureCategory_GUID', hidden: true, editable: false },
			{ text: languagePack.measures.grid1.MeasureDisplayName, editable: true, datafield: 'MeasureDisplayName' },
			{ text: languagePack.measures.grid1.MeasureTypeDisplayName, datafield: 'MeasureTypeDisplayName', editable: true, width: 200, columntype: 'dropdownlist',
				createeditor: function (row, column, editor) {
					// assign a new data source to the dropdownlist.
					var list = MeasureTypeNameList;
			...
			{ text: languagePack.measures.grid1.MeasureCategoryDisplayName, datafield: 'MeasureCategoryDisplayName', editable: true, width: 200 ,columntype: 'dropdownlist',
				createeditor: function (row, column, editor) {
					// assign a new data source to the dropdownlist.
					var list = MeasureCategoryNameList;
					editor.jqxDropDownList({ autoDropDownHeight: true, source: list });
				},
				// update the editor's value before saving it.
				cellvaluechanging: function (row, column, columntype, oldvalue, newvalue) {
			...
			{ text: languagePack.measures.grid1.IsLineupMeasure, datafield: 'IsLineupMeasure', columntype: 'checkbox', width: 75},
			{ text: 'Measure_IsActive', datafield: 'Measure_IsActive', editable: false, hidden: true },
	...
	$("#jqxgrid").on('cellendedit', function (event) {
		...
		if(value != oldvalue) {
			if(dataField == 'MeasureCategoryDisplayName') {
				rowData.MeasureCategory_GUID = MeasureCategoryGUIDList[MeasureCategoryNameList.indexOf(value)];
			}
			if(dataField == 'MeasureTypeDisplayName') {
				rowData.MeasureType_GUID = MeasureTypeGUIDList[MeasureTypeNameList.indexOf(value)];
			}
			if(dataField == 'MeasureDisplayName') {
				$("#jqxgrid").jqxGrid('setcellvalue', rowBoundIndex, "MeasureDisplayName", value);
			}
			if((dataField == 'MeasureDisplayName' || dataField == 'MeasureTypeDisplayName' || dataField == 'MeasureCategoryDisplayName' || dataField == 'IsLineupMeasure') && rowData.Measure_GUID != -1) {
