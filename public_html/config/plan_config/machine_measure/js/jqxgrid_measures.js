...
function LoadMeasuresGrid(url) {
	...
	var source = {
		datatype: "json",
		datafields: [
			{ name: 'MachineTypeMeasure_GUID',	display: 'MachineTypeMeasure_GUID',	type: 'string' },
			...
			{ name: 'MeasureName',				display: 'MeasureName',				type: 'string' },
	...
	$("#jqxgrid2").jqxGrid({
		...
		renderstatusbar: function (statusbar) {
			...
			addButton.click(function (event) {
				if(!editModeG2 && !lockedForService) {
					...
					var newRow = {};
					newRow.MachineTypeMeasure_GUID   = -1;
					newRow.MachineTypeDisplayname = machinetypeName;
					newRow.MachineType_GUID = machinetypeGuid;
		...
		columns: [
			...
			{ text: 'MachineTypeMeasure_GUID', datafield: 'MachineTypeMeasure_GUID', hidden: true, editable: false },
			...
			{ text: languagePack.machineMeasures.grid2.MachineTypeDisplayName, datafield: 'MachineTypeDisplayName', editable: false },
			{ text: languagePack.machineMeasures.grid2.MeasureCategoryDisplayName, datafield: 'MeasureCategoryDisplayName', editable: false },
			{ text: languagePack.machineMeasures.grid2.MeasureDisplayName, datafield: 'MeasureDisplayName', editable: true,  columntype: 'dropdownlist',
				...
				validation: function (cell, value) {
					if(!value) {
						return { result: false, message: languagePack.machineMeasures.invalidMeasure };
					}
					return true;
				}
			},
	...
	$("#jqxgrid2").on('cellendedit', function (event) {
		...
		if(value != oldvalue) {
			...
			if((dataField == 'MeasureDisplayName') && rowData.MachineTypeMeasure_GUID != -1) {
